  ...
  async function save() {
    try {
      ...
      if (!snapshot || typeof snapshot !== "object") {
        console.error(
          "[SaveLoadBar] invalid snapshot, aborting save",
          snapshot,
        );
        return;
      }
  ...
  async function load(id: string) {
    try {
      ...
      if (!data || typeof data !== "object") {
        console.warn(
          "[SaveLoadBar] wardrobe data malformed, using fallback empty object",
          data,
        );
        data = {};
      }
        ...
        <div className="max-h-40 overflow-auto text-sm space-y-1">
          ...
          {list.map((w) => (
            ...
            >
              ...
              <button
                onClick={() => load(w.id)}
                className="text-xs px-2 py-0.5 bg-neutral-200 rounded"
              >
                Učitaj
              </button>
            </div>
          ...
          {!loading && list.length === 0 && (
            <div className="text-xs text-muted-foreground">Nema sačuvanih.</div>
          )}
        </div>
      )}
    </div>
