...
const checkoutSchema = z
  .object({
    ...
    // Generated by the checkout dialog; the same key means the same order.
...
export async function POST(request: Request) {
  try {
    ...
    // Logged-in buyers count per account, so a shared IP can't block them.
    ...
    // A retry of an order that already went through (the buyer's connection
    // dropped before the answer arrived) gets that order back, not a new one.
    if (idempotencyKey) {
      const placed = await recallCheckout(idempotencyKey);
      if (placed) return NextResponse.json(placed, { status: 201 });
    }

    // A token Cloudflare refused blocks the order. If Cloudflare itself can't
    // answer, a real buyer shouldn't lose the order to its outage: it goes
    // through, the rate limit still applies, and admins see it flagged.
    const turnstile = await checkTurnstile(turnstileToken, identifier);
    ...
    if (turnstile === "unavailable") {
      console.error("Checkout accepted without Turnstile: siteverify down");
    }
    ...
    // =========================================================================
    // PHASE C: Answer the buyer, then send emails in the background
    // =========================================================================
    ...
    if (idempotencyKey) await rememberCheckout(idempotencyKey, placed);

    // The order is saved, so nothing below may fail or delay it. after() runs
    // once the response is sent; sendEmail never throws and retries transient
    // Resend errors, and the keys stop a retry from sending anything twice.
    after(async () => {
      ...
      try {
        if (mayEmailCustomer) {
          await sendOrderConfirmationEmail({
            ...
            shippingCity,
            shippingPostalCode,
            idempotencyKey: `order-confirmation/${txResult.orderId}`,
          });
        }
        ...
        await sendAdminNewOrderEmail({
          ...
          shippingCity,
          shippingPostalCode,
          unverified: turnstile === "unavailable",
        });

        // Invoice with the IPS QR code for payment
        if (mayEmailCustomer) {
          await sendInvoiceEmail({
            ...
            customerName,
            totalPrice: finalPrice,
            idempotencyKey: `order-invoice/${txResult.orderId}`,
          });
        }
      } catch (emailError) {
        // Rendering or the admin lookup failed; the order itself is saved.
      ...
      if (customerEmail) {
        await addToCustomersSegment(customerEmail, customerName.split(" ")[0]);
      }
    });
